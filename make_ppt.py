...
for i, (num, title, desc) in enumerate(toc_items):
    ...
    add_textbox(slide, Inches(2.2), y + Inches(0.02), Inches(4.0), Inches(0.42),
                title, size=22, bold=True, color=WHITE)
    add_textbox(slide, Inches(2.2), y + Inches(0.44), Inches(8.0), Inches(0.25),
...
div_headers = ["姓名", "分工内容"]
div_rows = [
    ["李彦恒", "总体设计 + 题目二排序算法（快排/选择排序）\n+ 索引数组实现 + 程序整合与测试"],
    ["陈召劲", "题目一：小猫钓鱼游戏\n链式队列与链式栈的设计实现 + 博弈逻辑编码"],
    ["云惟旺", "题目二：数据导入/查询/增速计算\n增加值分析 + 结果保存 + 实验报告撰写"],
]
